		...
		<div class="tete-page">
			<div class="row">
				...
				<div class="col-md-8" style="overflow: auto;padding-top: 50px" id="droite" align="center">
					...
					<titre> Destination commandes</titre>
					<br><br>
          <div class="place_loading" align="center"><div class='lds-spinner'><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div></div></div>
              ...
              <table class="table table hover">
                ...
                //COMMANDES PAR AGENCES
                  ...
                  $commande = $this->Admin_model->tout_commande();
                    foreach ($commande as $key => $ag) {
                      $lat_long = explode(",", $ag['latlng_v']);
                      $lat_long_a = explode(",", $ag['coord_latlng_a']);
                      
                      $array_dest[] = array("lat" => $lat_long[0], "lon" => $lat_long[1],"lat_a" => $lat_long_a[0], "lon_a" => $lat_long_a[1], "id_vente" => $ag['id_vente'], "id_agence" => $ag['id_agence']);
                    }
                  ...
                  foreach ($agences as $key => $ag) {
                    ...
                    $array[] = array("lat" => $lat_long[0], "lon" => $lat_long[1], "id_agence" => $ag['id_agence'], "nom_a" => $ag['nom_a']);
                  ?>
                    <tr>
                      <td><a href="<?php echo base_url() ?>bord/commande_par_agence?p=<?php echo $ag['id_agence'] ?>" target="_blank"> <?php echo $ag['nom_a'] ?></a> </td>
                  </tr>

                  <?php
                  }
                 $geojson = json_encode($array);
                 $geojson_dest = json_encode($array_dest);
             ...
             <script>
                ...
                var centrer = [-21.46342, 47.11029];
                  ...
                  var items_dest = <?php echo $geojson_dest ?>;
                 // console.log(items);
                  ...
                  for(i=0;i<items.length;i++){
                      var LamMarker = new L.marker([items[i].lat, items[i].lon], {
                        ...
                        ' <div class=row><div class=col-md-12><div class=media> <div class=media-body>'+items[i].nom_a+'<br><a href=<?php echo base_url() ?>bord/commande_par_agence?p='+items[i].id_agence+' target=_blank>Voir details</a></div></div></div></div></div>',{maxWidth:200}

                        );
                      }

                      //DESTINATIONS COMMANDES
                      for(j=0;j<items_dest.length;j++){
                      L.marker([items_dest[j].lat, items_dest[j].lon]).addTo(map).bindPopup(

                        ' <div class=row><div class=col-md-12><div class=media> <div class=media-body><a href=<?php echo base_url() ?>bord/commande_details?p='+items_dest[j].id_vente+' target=_blank>Voir details</a></div></div></div></div></div>',{maxWidth:200}

                        );
                      L.polyline([[items_dest[j].lat_a, items_dest[j].lon_a], [items_dest[j].lat, items_dest[j].lon]], {
                              color: 'green'
                            }).addTo(map);
                      }
                  }
