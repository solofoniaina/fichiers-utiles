				...
				<div class="col-md-8" style="overflow: auto;padding-top: 50px" id="droite">
					...
					<BR><br>
						...
						<?php 
							...
							//CAS MODIF LOCALISATION
							if (!empty($id_agence_modif)) {
								$latlng_amod = $this->Agence_model->get_latlng($id_agence_modif);
								$latlng_modif = explode(",", $latlng_amod);
							}
							else $id_agence_modif = "";
								...
								<script type="text/javascript">
									...
									*/
									...
									/*create array for markers & polyline*/
                  					...
                  					//cas modification localisation
									var marker_modif = "<?php echo $id_agence_modif ?>";
									if (marker_modif != "") {
										var mrkr = L.marker([<?php echo $latlng_modif[0] ?>, <?php echo $latlng_modif[1] ?> ]);
										mks1.push(mrkr);
                      					mks1[0].addTo(map).bindPopup(

										'	<div class=row><div class=col-md-12> <div class=media-body> <h4 class=media-heading></h4>Eto isika no maka ny entana</div></div></div></div></div>',{maxWidth:200}
